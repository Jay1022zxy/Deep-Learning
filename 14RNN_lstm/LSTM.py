...
# 在数据准备阶段已经运行过一次了，这里直接加载训练好的 BPE 模型。
...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):         # 获取指定索引处的英文和中文句子对，并返回它们的token ID列表作为LongTensor
...
class Encoder(nn.Module):
    def __init__(self, vocab_size, emb_dim, hid_dim, n_layers=2):
        super().__init__()
        # n_layers是Encoder的层数，配置原因选择2层双向LSTM。每一层的输入和输出维度都是hid_dim。
...
# Decoder部分的输入维度和输出维度都比较特殊，注意力机制的引入使得Decoder在每个时间步的输入不仅包含当前时间步输入token的embedding，
# 还包含了一个注意力上下文向量。
...
# 定义Seq2Seq模型，包含一个Encoder和一个Decoder。Seq2Seq的输入是英文token序列，输出是中文token序列。
...
def train(model, iterator, optimizer, criterion):
    ...
    torch.save(model.state_dict(), '14RNN_lstm/seq2seq_bpe_attention.pt')  # 每个epoch结束后保存模型参数
