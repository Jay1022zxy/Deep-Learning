...
# 整体思想：
# 使用1x1卷积进行降维，减少计算量；使用3x3卷积进行特征提取；再使用1x1卷积进行升维，恢复通道数。
# 每个卷积后跟 BN 和 ReLU。若输入输出维度不一致，则在捷径路径使用 1x1 卷积。

expansion = 4            # 扩展系数，表示输出通道数相对于输入通道数的倍数

class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        super(Bottleneck,self).__init__()
        # 第一个卷积层，使用1x1卷积核，步长为1，填充为0，用于降维
        self.cov1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, 
        ...
        # 第二个卷积层，使用3x3卷积核，步长和填充根据参数设置，用于特征提取
        self.cov2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, 
                              stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels)                      # 第二个卷积层后的批归一化层

        # 第三个卷积层，使用1x1卷积核，步长为1，填充为0，用于升维
        self.cov3 = nn.Conv2d(out_channels, out_channels * self.expansion, kernel_size=1, 
                              stride=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(out_channels * self.expansion)     # 第三个卷积层后的批归一化层
    ...
    def forward(self, x):                            # 定义前向传播方法，接受输入数据并返回输出
        ...
        out = self.cov3(out)                         # 通过第三个卷积层处理数据
        ...
        out = self.relu(out)                         # 通过ReLU激活函数处理相加
